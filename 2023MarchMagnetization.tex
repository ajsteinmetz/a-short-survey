...
\begin{figure}[h]
  ...
  \caption{Plot of the down quark chemical potential (black), electron chemical potential (dotted red) and neutrino chemical potential (dashed green) as a function of time. (2003 unpublished, Fromerth \& Rafelski)}
  \label{QGPchem1} 
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{enumerate}
...
In this case, bottom flavor breaks the detail balance and disappearance from particle inventory during the epoch $T = 0.3 \sim0.15$ GeV which is the essential condition for departure from the thermal equilibrium. Our results provide a strong motivation to explore the physics of baryon nonconservation involving the bottom quarks or bottomnium mesons  in thermal environmen. Given that the non-equilibrium of bottom flavor arises at relatively low QGP temperature allows the possibility for baryogenesis to be occurred in primordial- QGP hadronization {\xlbue add reference}.
...
Fig \ref{taumupi}(on left) shows the invariant production rates $R$ of pion(on right) and relaxation time (on left) for the different processes considered as function of temperature $T\in [3,50]$ MeV. The general form for invariant production rates and relaxation time is discussed in paper \cite{Kuznetsova:2008jt}. It shows that $\gamma+\gamma\to \pi^0$(solid blue line) is the dominant mechanism of pion production. The other solid line with dots corresponds to $e^++e^-\to \pi^0$ reaction which in essence remains, in comparison, insignificant. Its importance follows from the fact that it provides the second most dominant path to $\pi_0$ formation at lowest temperatures considered.

Since the  $\gamma+\gamma\to \pi^0$ is the dominant mechanism of pion production, then we can ask question: at what temperature in the expanding early Universe does this reaction
`freeze' out, that is the $\pi^0$ decay overwhelms the production rate and the yield
falls away from chemical equilibrium yield. To answer this question, we study the population equation of $\pi^0$ which describe the evolution of $\pi^0$ number density in early universe \cite{Fromerth:2012fe}. In Fig.(\ref{taumup}) (on right) we compare the relaxation time of $\tau_{\pi^0}$ to the Hubble time $1/H$, it shows that $\tau_{\pi^0}\ll 1/H$. In such case, the yield of $\pi^0$ is expected to be chemical equilibrium. Since the decay rate is compensated by the production rate, and within $100$ as, the chemical equilibrium yield is attained even as its thermal number density gradually decreases. This phenomenon can be attributed to the high population of photons. In such environment, it remains probable to find high energy photons to fuse into $\pi^0$ in early universe. 
%~~Figure~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\begin{figure}[h]
%\includegraphics[width=0.47\columnwidth]{nmupi.pdf}
\includegraphics[width=0.47\columnwidth]{pions1.pdf}
\includegraphics[width=0.47\columnwidth]{taupi0.pdf}
...
\end{align}
where $U$ is the PMNS mixing matrix. The PMNS matrix is the lepton equivalent to the CKM mixing matrix which describes the misalignment between the quark flavors and their masses. The parameter $\delta$ is the CP-violating phase which is present when the number of generations is at least three. In principle, the number of mass eigenstates can exceed three, but is restricted to three generations in most models. By standard convention found in the literature we parameterize the rotation matrix $U$ as
	\begin{alignat}{1}
  	\label{PMNS} U =
		\begin{pmatrix}
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=\linewidth]{Chemical_Potential_Neff002}
...
\begin{itemize}
    ...
    \item {\xred Note that CPT symmetry is protected by the lack of asymmetry in normal SM reactions to some large factor by the accumulation of scattering events. CPT is similarly restricted by the mass difference in the Kaons via the difference in strange-antistrange masses.}
\end{itemize}
\reftitle{References}
