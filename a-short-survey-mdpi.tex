...
\noindent At an early time in the standard cosmological model, the Universe began as a fireball, filling all space, with extremely high temperature and energy density~\cite{Rafelski:2015cxa}. {\xblue Our domain of present day Universe originated from an ultra-relativistic plasma which contained almost a perfect symmetry between matter and antimatter except for a small discrepancy of one part in $10^{9}$ which remains a mystery today. There are two general solutions of this problem which both suppose that the  Universe initial conidtions were baryon-antibaryon number symmetric as it is generally believed in order to avoid `fine-tuning' these initial conditions to a specific current value:
\begin{itemize}
\item[A] Case of baryonic number (charge)  conservation: We  need a `force' capable during the Universe evolution to create this asymmetry by matter-antimatter seperation. Two of the three Sakharov~\cite{Sakharov:1967dj,Sakharov:1988vdp} conditions must be fullfilled:
\begin{itemize}
...
\end{itemize}
Missing antimatter could be `stored' in a local even singular  antimatter structure, while matter is distributed more uniformly as we see it around us today. We recall the standard wisdom that the known CP-violation in the Standard Model's weak sector is very probably insufficient to explain the baryon asymmetry we see today~\cite{Rubakov:1996vz}.
\item[B] Since there is nothing we know that protects baryon charge it is possible to consider the full Skharov model with 
\begin{itemize}
...
{\xblue Notwithstanding the commonly made presumption that in our cosmic neighborhood there are no antibaryon domains remaining, we note the continued work by M. Yu. Khlopov and collaborators addressing the opposite scenario and refer the interested reader to a recent review~\cite{Khlopov:2023dbg}. However,  in this work we take a homogneous prescribed baryon asymmetry  obtained from baryon  to photon ratio in the Universe. Sdditional comments on the situation in the context of non-equilibria processes are made in \rsec{sec:BottomCharm},  at the end of \rsec{sec:Muons},  and in \rsec{sec:Summary}. 
...
\noindent {\xblue As seen in \rsec{sec:Strangeness}, the muon abundance and their reactions are integral to the understanding of the strangeness content of the primordial Universe~\cite{Yang:2021bko}. Therefore we determine to what extent and temperature muons remained in chemical abundance equilibrium. Without a clear boundary separating the hadronic epoch from the leptonic epoch, there is complete overlap in the hadronic and leptonic species dynamics in the period $T=\mathcal{O}(10\MeV)\sim\mathcal{O}(1\MeV)$.}
...
\end{align}
{\xblue Provided that all particles shown on the right-hand side of each reaction (namely the photons, electrons(positrons) and charged pions) exist in chemical equilibrium, the back-reaction for each of the above processes occurs in detailed balance.}
...
\end{equation} 
with the vacuum life time $\tau_{\mu}=2.197 \times 10^{-6}$ seconds. {\xblue We recall the considerable shorter vacuum lifetime of pions  $\tau_{\pi^\pm}=2.6033\times10^{-8}$ seconds.} The thermal decay rate per volume in the Boltzmann limit is~\cite{Kuznetsova:2008jt}
\begin{align}
&R_i=\frac{g_i}{2\pi^2}\left(\frac{T^3}{\tau_i}\right)\left(\frac{m_i}{T}\right)^2K_1(m_i/T) 
\end{align}
where $\tau_i$ is the vacuum lifespan of a given particle $i$. {\xblue These production and decay rates for muonic processes are evaluated in~\cite{Rafelski:2021aey}. From this, we can determine the temperature when muons rather suddenly disappear from the particle inventory of the Universe occuring when their decay rate exceeds their production rate.} 
